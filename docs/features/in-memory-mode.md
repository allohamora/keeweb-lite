...
- Keep unlocked database model in runtime app state (in-memory, non-persistent) while session is active.
...
- Runtime app state (in-memory, non-persistent): unlocked model and transient editing state.
