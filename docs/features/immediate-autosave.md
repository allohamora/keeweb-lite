...
- Source-specific persistence:
  ...
  - Drive-backed: sync via Drive adapter, use 2-way merge on remote changes/rev conflicts, and update sync fields (`rev`, `syncDate`, `syncError`, `syncing`)
...
- Persisted targets depend on source adapter and configuration:
  ...
  - Drive-backed metadata: Internal App Storage (localStorage) file-info fields, including `rev` and `syncDate`; runtime model holds `syncing` and `syncError`
  - Drive OAuth runtime token data: browser `localStorage` key `keeweb-lite.oauth.google-drive` (cleared on `logout` or invalid refresh-token path)
...
- Failed Drive sync retries must preserve previous successful `syncDate` value.
